  ...
  class ThirdPartLogin {
    ...
    async thirdPlatformCallback(code, state, callback) {
      ...
      try {
        ...
        } else if (data.processCode === 4) {
          window.location.href = `${redirectTo(SIGN_IN)}?form_value=${encodeURIComponent(JSON.stringify(data.uinfo || {}))}`;
        }
      } catch (e) {
        hideLoading();
...
window.SLM['theme-shared/biz-com/customer/biz/sign-in/index.js'] = window.SLM['theme-shared/biz-com/customer/biz/sign-in/index.js'] || function () {
  ...
  class Login extends Customer {
    ...
    async initForm() {
      ...
      let formValue = storage.sessionStorage.get(DEFAULT_FORM_VALUE);
      ...
      try {
        defaultValue = JSON.parse(form_value);
      } catch (error) {}
      if (defaultValue) {
        if (this.activationTag) {
          const {
            countryCode,
            nationNumber,
            email
          ...
          if (mode === 'username') {
            ...
            } else if (nationNumber) {
              formValue = {
                [accountFieldType]: nationNumber,
                iso2: countriesDialCodeMap[countryCode]
              };
            }
          } else if (mode === 'phone') {
            formValue = {
    ...
    onSignIn(data) {
      ...
      const onLogin = (captchaToken, updateParams = {}, serialNo = '') => {
      ...
      return wrapArmorCaptcha({
        ...
        onCaptchaVerifySuccess: (captchaToken, prevRequestResult, serialNo) => onLogin(captchaToken, {
